...
@pytest.mark.xfail(reason='Fails because of clisops subset: see flamingo issue: #82')
...
def test_run_tree_wf_4():
    ...
    wf = workflow.TreeWorkflow(output_dir=tempfile.mkdtemp())
    output = wf.run(wfdoc)
    assert os.path.basename(output[0]) == 'rlds_Amon_IPSL-CM6A-LR_historical_r1i1p1f1_gr_19850116-20141216.nc'
...
def test_run_tree_wf_5():
    ...
    wf = workflow.TreeWorkflow(output_dir=tempfile.mkdtemp())
    output = wf.run(wfdoc)
    assert os.path.basename(output[0]) == 'rlds_Amon_IPSL-CM6A-LR_historical_r1i1p1f1_gr_19950116-20001216.nc'
